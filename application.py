...
def upload():
    smr = request.form.get('smr')
    formWords = request.form.get('words')
    print("THESE ARE THE FORM INPUTS")
    print("smr checked: " )
    print(smr)
    print("words: ")
    print(formWords)

    if smr == "shall/must/required":
        if formWords:
            formWords = smr + "/" + formWords
        else:
            formWords = smr
    words = formWords.split("/")
    discount_string = "Minister/Director/Provincial Officer"
    discount_bank = discount_string.split("/")
    print(discount_bank)
    #the folder to store the sent files
    currentDT = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S:%f")
    print (str(currentDT))
    target = os.path.join(APP_ROOT, ('files/' + currentDT))

    #make the file folder if it doesn't exist
    if not os.path.isdir(target):
        os.mkdir(target)

    #save the files into the destination folder on the server
    for file in request.files.getlist("file"):
        name = file.filename.split("/")
        filename = name[len(name)-1]
        if not ".DS_Store" in file.filename:
            print("THE NEXT LINE IS A FILE" + file.filename)
            #print(file.name)
    ...
    for file in files:
        if not file.endswith(".docx"): 
            text_file.write("\n Warning the file: " + file + " will not be processed and was not included in the downloaded zip, as it is not in the .docx Format. \n\n")

    files = [x for x in files if (x.lower().endswith(".docx"))]
        ...
        if (file != '.DS_Store'):
            ...
            for paragraph in document.paragraphs:
                ...
                for d in discount_bank:
                    if d.lower() in paragraph.text.lower():
                        discount = True
                        ...
                        for i, run in enumerate(runList):
                            ...
                            if word.lower() in run.text.lower():
                                ...
                                for i in range(len(split_runs)-1):
                                    if re.search("\\b(" + word + ")\\b(?i)", split_runs[i].lower()):
                                        #print(split_runs[i] + split_runs[i + 1])
                                        newRun = add_run_copy(paragraph, run, split_runs[i])
                                        font = newRun.font
                                        if discount:    
                                            font.highlight_color = WD_COLOR_INDEX.GRAY_25
                                        elif re.search("\\b(be)\\b(?i)", split_runs[i + 1].lower()):
                                            font.highlight_color = WD_COLOR_INDEX.GRAY_25
                                            print(split_runs)
                                        elif (idx == 0):
    ...
    #setting up paths for src and dst of zip folder
    file_names = os.listdir(target)
    file_paths = []
    arcnames = []
    ouput = os.path.join(APP_ROOT, 'Output')
    copyfile((APP_ROOT + "/output-stats.txt"), (ouput + "/output-stats.txt"))
    print(ouput + "/output-stats.txt")

    for file_name in file_names: 
        file_paths.append("/".join([ouput, file_name]))
        arcnames.append("highlighted/" + file_name)
    file_paths.append("/".join([ouput, 'output-stats.txt']))
    arcnames.append("highlighted/" + 'output-stats.txt')

  
    # writing files to a zipfile 
    with ZipFile('highlighted.zip','w') as zip: 
        # writing each file one by one 
        for idx, file in enumerate(file_paths): 
            print("zipping: " + file)
            zip.write(file, arcnames[idx]) 


    returnzip = os.path.join(APP_ROOT, 'highlighted.zip')
    return send_file(returnzip)
